...
enums automatically receives generated JSON codecs. Marked enums (`func (T)
enum()`) in their default value mode need no codec and keep ordinary Go JSON
behavior. The run writes `types.ts` and, when requested, `index.ts` alongside
