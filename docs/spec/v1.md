...
- String (opt‑in): add WithEnumMode(EnumStrings) after WithEnum(…). JSON Schema enum is strings.
  - Name resolution order: WithEnumName override > const identifier. (T.String() support is a post‑v1 add‑on.)
  - (Adapters for (un)marshal are planned as a post‑v1 add‑on; current v1 focuses on schema emission.)
