...
export type Proof = [
  ...
  Assert<Equal<Envelope["status"], "ready" | 'wait"ing'>>,
  Assert<Equal<Envelope["priority"], 0 | 1>>,
  Assert<Equal<Envelope["priority_name"], "Low" | "High">>,
...
const envelope: Envelope = {
  event: created, other: { "other-key": 'create"雪', name: "Ada" },
...
// @ts-expect-error the singleton's field-local key is required
const wrongLocalKey: Envelope["other"] = created;
...
void [missingTag, wrongTag, wrongPayload, optionalNull, optionalUndefined,
  missingNullable, wrongEnum, wrongName, wrongLocalKey, nullBranch, emptyNumber, emptyNull];
