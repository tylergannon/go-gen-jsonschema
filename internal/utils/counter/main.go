...
func measureSchema(schema interface{}) int {
	var total int
	...
	case map[string]interface{}:
		...
		// Check for "enum"
		if enumVals, ok := node["enum"].([]interface{}); ok {
...
func main() {
	...
	responseFormat, ok := root["response_format"].(map[string]interface{})
	if !ok {
		...
		if !ok {
			...
			schemaObj, ok := jsonSchema["schema"].(map[string]interface{})
			if !ok {
				fmt.Println("Structured Response Length: 0")
			} else {
	...
	if functionsVal, ok := root["functions"].([]interface{}); ok {
		for i, fn := range functionsVal {
			...
			if parameters, ok := fnObj["parameters"].(map[string]interface{}); ok {
